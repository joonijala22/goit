...
class Comment
{
	...
	public static function from_post($content_page, $id, $datetime)
	{
		...
		return new Comment($content_page, $id, $name, $email_address, $website, $message, $datetime, $is_preview);
	}
	...
	public static function form_page($page) {
		try {
			...
			return new Comment(
				...
				$website,
				$page->text()->value(),
				new DateTime(date('c', $page->date()->toDate()))
			);
		} catch (Exception $e) {
	 ...
	 */
	function __construct($content_page, $id, $name, $email_address, $website, $message, $datetime, $is_preview=false)
	{
	...
	public function message()
	{
		...
		if (kirby()->option('rastatux.kirbycomments.form.message.htmlentities')) {
			$message = htmlentities($message);
		}

		$message = markdown($message);

		if (kirby()->option('rastatux.kirbycomments.form.message.smartypants')) {
			$message = smartypants($message);
		}

		return strip_tags($message, kirby()->option('rastatux.kirbycomments.form.message.allowed_tags'));
	}
