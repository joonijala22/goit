...
File-based comments stored as subpages, working now with Kirby v3! Copied and Updated from AddPixels work, see:
...
```json
  "repositories": [
    {
      "type": "git",
      "url": "git@gitlab.com:rastatux/kirbycomments.git",
      "reference": "master"
    }
  ]
```

2. Requite the package:

```json
"rastatux/kirbycomments": "^1.5.6"
```

### Manually

1. Download as a zip-file.
2. Decompress the zip file and rename the folder to *kirbycomments*.
3. Move the folder *kirbycomments* into `site/plugins`.

When updating, simply delete the old *kirbycomments* folder and replace it with the new one.
...
### Nothing happens after sending the Comment

Probably the Plugin can't create the `comments` folder in your page. Check if the user that runs your site is able
to create folders and files within your website root. One solution could be to use Continuous Integration to deploy the page with the right owner.
It's not recommended due to security - but a quick & dirty step would be to set the permissions to 777.

## Options

Set options by calling `c::set('comments.OPTION_NAME', $value)` in your `site/config/config.php`.
