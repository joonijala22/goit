      ...
      <nav class="breadcrumbs" aria-label="Breadcrumbs">
        <ol class="flex flex-wrap gap-xxs">
          <li class="breadcrumbs__item">
            <a href="<?= $pages->find('news')->url() ?>" class="color-inherit">Community</a>
            <span class="color-contrast-low margin-left-xxs" aria-hidden="true">/</span>
          </li>
          
          <li class="breadcrumbs__item color-primary" aria-current="page">Workshop</li>
        </ol>
      </nav>
    </div>
  </section>
...
$p2 = page('news');         
  $filterBy = get('filter');
  $firstblogs = $p->children()->listed()->sortBy('date', 'asc')->filterBy('tags', 'workshop', ',')->flip()->limit(3);
  $lblogs = $p2->children()->listed()->sortBy('date', 'asc')->filterBy('tags', 'workshop', ',')->flip()->offset(3)->paginate(3);
?>
